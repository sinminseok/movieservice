...
public class MovieRepository {
    private static final int MIN_SCHEDULE_LENGTH = 0;
    ...
    public static Movie findById(int id) {
        return movies.stream()
                .filter(movie -> movie.equalId(id))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException());
    }

    public static boolean existPlaySchdule(Movie movie, int playSchedule) {
        if (MIN_SCHEDULE_LENGTH < playSchedule && playSchedule < movie.getPlaySchedules().size()) {
            return true;
        }
        return false;
    }
